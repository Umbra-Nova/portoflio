...
export class ProductPage extends PrestaShopBasePage {
  ...
  async clickFirstProduct(): Promise<void> {
    // Ensure the home page has finished loading network requests.
    await this.page.waitForLoadState('networkidle');
    // Give the product grid a moment to render.
    ...
    // Force click to avoid overlay interference.
  ...
  async verifyVariantsSelected(): Promise<void> {
    ...
    if (await sizeSelector.isVisible().catch(() => false)) {
      const sizeValue = sizeSelector;
      await expect(sizeValue).toHaveValue();
    }

    if (await colorSelector.isVisible().catch(() => false)) {
      const colorValue = colorSelector;
      await expect(colorValue).toHaveValue();
    }
  }
